...
namespace InstallDB
{
    ...
    public partial class installDB : Installer
    {
        ...
        private string strDBName;       //要建数据库的名称,默认为Logistics

        private string strDBServerName; //数据库服务器名称

        private string strDBUser;       //数据库登录名

        private string strDBPwd;        //数据库密码

        private string strPhysicalDir;  //安装的物理路径

        private string strDBPath;       //把数据库创建在何处
        ...
        public override void Install(IDictionary stateSaver)
        {
            try
            {
                ...
                strSetupContentIndex = this.Context.Parameters["BTNSETUPCONTENT"].ToString().Trim();    //要安装的内容,1:安装信息系统和数据库;2:只安装信息系统(不安装数据库);3:只安装数据库
        ...
        private void RegistDotNetIIS()
        {
            ...
            { 
                MessageBox.Show("注册.NET到IIS失败,请安装完毕后手动注册！","安装提示", MessageBoxButtons.OK, MessageBoxIcon.Error, MessageBoxDefaultButton.Button1,0);
            }                    
        }
        #endregion
        ...
        #region 将主页添加到收藏夹和在桌面添加收藏夹
        private void AddFavorites()
        {
            try
            {
                System.IO.File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.Favorites) + @"\卷烟分拣信息管理系统.url");
                ...
                shortcut = iwshshell.CreateShortcut(path + @"\卷烟分拣信息管理系统.url") as IWshURLShortcut;
                shortcut.TargetPath = @"http://localhost/SortingSystem/";
                shortcut.Save();


                System.IO.File.Delete(Environment.GetFolderPath(Environment.SpecialFolder.Desktop) + @"\卷烟分拣信息管理系统.url");
